// ICG coded in RTL instead of inference by Genus
...
module cache_2way_clock_gated (
  ...
  // Return word or zero-padded byte
  wire [31:0] hit_data =
                req_size ? hit_word :
                           {24'b0, hit_byte};
  ...
  // Miss-context registers clock enable
  wire miss_ctx_clk_en =
              miss_accept_en;

  // Memory-response register clock enable
  wire mem_resp_clk_en =
              (state == MISS_WAIT) &&
              mem_resp_valid;

  // Cache-array clock enable
  ...
  logic miss_ctx_clk;
  logic mem_resp_clk;
  logic cache_array_clk;
  logic lru_clk;

  icg_latched u_icg_fsm (
      .clk   (clk),
      .en    (fsm_clk_en),
      .rst_n (rst_n),
      .gclk  (fsm_clk)
  ...
  icg_latched u_icg_cache_array (
      .clk   (clk),
      .en    (cache_array_clk_en),
      .rst_n (rst_n),
      .gclk  (cache_array_clk)
  );

  icg_latched u_icg_lru (
      .clk   (clk),
      .en    (lru_clk_en),
      .rst_n (rst_n),
      .gclk  (lru_clk)
  ...
  always_ff @(posedge miss_ctx_clk or negedge rst_n) begin
  ...
  always_ff @(posedge mem_resp_clk or negedge rst_n) begin
  ...
  // Cache arrays
  always_ff @(posedge cache_array_clk or negedge rst_n) begin
  ...
  // LRU register
  always_ff @(posedge lru_clk or negedge rst_n) begin
